...
class ConversionControllerIntegrationTest {
	...
	@Test
	void whenTestGetPurchaseConversions_thenReturnConversionDTO() throws Exception {
		String purchaseDescription = "ConversionIntegrationFull";
		PurchaseDTO savedPurchase = purchaseService.addPurchase(purchaseDescription, 5.0, LocalDate.now());

		webClient.get().uri("/api/v1/conversion/{id}", savedPurchase.getId()).accept(MediaType.APPLICATION_JSON)
